...
StochasticSamples* allocateStochasticSamples(RNA* strand, int numSamples) {
  ...
  newSamples->structures = (Structure*) malloc(numSamples * sizeof(Structure));
...
Structure allocateStructure(RNA* strand) {
  return (Structure) malloc(strand->length * sizeof(int));
}
	...
	if (top->matrix == 0) /* Zb */
	  {
	    /* bp[i - 1] = j; */
	    /* bp[j - 1] = i; */
	    pair(strand->samples->structures[sample], i, j);
