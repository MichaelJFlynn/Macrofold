...
#define BUFLENGTH   1000  // how many entries are we allowing per line
#define BUFHEIGHT 10000 // how many lines we are allowing

Line* parseLine(char* line, char* tokens) {
  char** parsedLine;
  char* tokenHolder[BUFLENGTH];
  char* token;
  int i, numTokens = 0;
...
DataFile* constructDataFile(Line** lines, int nrow) {
  ...
  for(i = 1; i < nrow; i++) {
    if(lines[i]->length != df->header->length) {
      printf("Inconsistent column number: %d, %d\n", lines[i]->length, df->header->length);
    }
    df->data[i-1] = lines[i];
  }
  return df;
