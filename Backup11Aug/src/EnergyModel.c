...
void initializeEnergyModel(RNA* strand) {
  ...
  // Initialize the multibranch constants e^{-\Beta a}, etc.
  ...
  // Initialize all the 'AU' penalties
  for(i = 0; i < 4; i++) {
    for(j = 0; j < 4; j++) {
  ...
  strand->energyModel->auPenalty[baseMap("A")][baseMap("G")] =  
    exp(1.1 / (R * strand->temperature));
  // WHY IS THERE A PENALTY FOR 'U' AND 'U'?
  strand->energyModel->auPenalty[baseMap("U")][baseMap("U")] =  
    exp(0.7 / (R * strand->temperature));

  // I DON'T UNDERSTAND HOW THIS SCALE FACTOR STUFF WORKS
...
/************************************************
 Here we load up all the different energy tables
************************************************/
